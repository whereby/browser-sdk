...
import { Box, CenterGridLayout, makeBox } from "../../helpers/layout";
...
const hasDuplicates = (arr: AspectRatio[]) => new Set(arr).size !== arr.length;

const findMostCommon = (arr: AspectRatio[]) => {
...
export function pickCellAspectRatio({ choices = [] }: { choices: AspectRatio[] }) {
    // If all cells are the same aspect ratio use that:
    const minAr = Math.min(...choices) as AspectRatio;
    const maxAr = Math.max(...choices) as AspectRatio;
    let chosenAr = null;
    if (minAr === maxAr) {
        chosenAr = minAr;
    } else {
        ...
        chosenAr = clamp({ value: dominantAr ?? maxAr, min: AspectRatio.Normal, max: AspectRatio.Wide });
    }
    return {
        minAr,
        maxAr,
...
export function calculateLayout({
    ...
    cellCount,
    gridGap,
    cellAspectRatios = [AspectRatio.Normal],
    paddings = makeBox(),
}: {
    width: number;
    height: number;
...
export function getCellPropsAtIndexForLayout({ index, layout }: { index: number; layout: CenterGridLayout }) {
